...
library(stringr) #helps in manipulating string vars
```
...
Add a column to identify each camera trap actual camera trap by extracting the camera trap, card number and forest information from each image name.Then paste that to round number.
Requires package `stringr`

```{r}
#requires stringr
Sampling.Unit.Name<-str_sub(masterDF$Imj1, start = 1, end = 9) #use 9 because some forests have a 3 letter rather than 2 letter code.
#now paste to round number
masterDF$Sampling.Unit.Name<-paste0(Sampling.Unit.Name, masterDF$round)

```
Now add a sampling year (the whole data set). We can think of sampling event as equal to round.
```{r}
masterDF$Sampling.Year<-19.20
```
...
Now let's visualize camera days
```{r}
...
#math check. Does the sum of effort_per_round$total_cam_days equal the total_cam_days summed above?

sum(effort_per_round$total_cam_days)
#Yes!
```
#### Now try with CTFWR code
This gives different answers when I run it different times, which makes me suspect.  I'm going to use our code (above) for calculating camera effort.
...
The second time I ran this, I got 75 instead of 74 (and not 77!). What gives? It is not that I needed to add round number to the Sampling.Unit.Name because I did that in the second pass.
```{r}
# library(compareDF)
# library(htmlTable)
# test1<-cam_days
...
The note above I fixed in the second round of running this code and I still see an issue when I run compare_df with camera 31_03_Wh3.  Not sure what the problem is, so I'm just going to keep using my own code.

#### Events per time
Derive, for each species and camera trap station, the trapping events according to desired time-interval sets.  The CTFWR has a function, event.sp that does this. Let's give it a whirl.To do so, we need to rename the columns in masterDF to match those in CTFWR `data` object.

```{r}
CTFWR<-masterDF
names(CTFWR)
```
Reassign names to match CTFWR key columns

```{r}
newnames<-c("subject_ids", "diff_species", "choice", "Imj1", "Imj2", "Img3", "CamNum", "SD_card_num", "ForestType", "ForestName", "round", "Camera.Start.Date.and.Time", "Date_Checked", "Camera.End.Date.and.Time", "Sampling.Unit.Name", "Sampling.Event", "td.photo", "Photo.Time", "Photo.Date")

names(CTFWR)<- newnames
```

```{r}
events_hh<-event.sp(dtaframe = CTFWR, year = 19.20, thresh = 60)
```
Not working, and also not critical, so come back to CTFWR chapter 5. p. 56 later.


### Species Accumulation
The CTFWR code calls their function `acc.curve` which in turn calls their function `f.matrix.creator` and also some `vegan` functions. I'm going to try and step through it without the functions, just running the code.

```{r}
library(reshape)
library(vegan)
```
Now step into their `acc.curve` function:

```{r}
yr<-masterDF
```
Next step into `f.matrix.creator` as part of `acc.curve`

```{r}
res<-list() #create results object
#list sampling units
cams<-unique(masterDF$Sampling.Unit.Name)
cams<-sort(cams)
rows<-length(cams)
#start and end dates of sampling periods
min<-min(masterDF$Date_Out)
max<-max(masterDF$Use_End_Date)
cols<-max-min+1

#sampling period
date.header<-seq(from = min, to = max, by = 1)
mat<-matrix(NA, rows, cols, dimnames = list(cams, as.character(date.header)))
#produced empty matrix with a row for each camera and a column for each date from the date the first camera was out to the date the last camera was brought in.

#for all cameras, determine the open and close date and mark in the matrix



start.dates<-tapply(as.character(masterDF$Date_Out), masterDF$Sampling.Unit.Name, unique)

new.start.dates<-integer(0)
class(new.start.dates)<-"Date"

for(z in 1:length(start.dates)){
  new.start.dates[z]<- start.dates[[z]]
}
start.dates<-new.start.dates


end.dates<-tapply(as.character(masterDF$Use_End_Date), masterDF$Sampling.Unit.Name, unique)
new.end.dates<-integer(0)
class(new.end.dates)<-"Date"

for (z in 1:length(end.dates)){
  new.end.dates[z]<-end.dates[[z]]
}
end.dates<-new.end.dates
...
mat.template<-mat
  #get the species
species<-unique(masterDF$choice)
#construct the matrix for each species, i
for(i in 1:length(species)){
  indx<-which(masterDF$choice == species[i])
  #dates and cameras when/where the species was photographed
  dates<-masterDF$Photo.Date[indx]
  cameras<-masterDF$Sampling.Unit.Name[indx]
  dates.cameras<-data.frame(dates, cameras)
  #unique combination of dates and cameras
  dates.cameras<-unique(dates.cameras)
  #convert the date column in dates.cameras to date
  dates.cameras$dates<-mdy(dates.cameras$dates)
...
names(res)<-species
```
Ok, that took care of the `f.matrix.creator` step inside of `acc.curve`.  Keep going.
```{r}
pr<-melt(mat)
